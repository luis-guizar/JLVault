...
import 'package:simple_vault/services/vault_manager.dart';
...
/// Tests for premium feature integration
/// These tests verify that premium features work correctly when integrated
void main() {
  group('Premium Feature Integration Tests', () {
    group('Multiple Vaults Integration', () {
      ...
      test('Vault switching maintains separate data', () {
        // Test that vaults maintain separate encryption and data

        final workVault = VaultMetadata.create(
          name: 'Work Vault',
          iconName: 'work',
          color: Colors.blue,
        );

        final personalVault = VaultMetadata.create(
          name: 'Personal Vault',
          iconName: 'home',
          color: Colors.green,
        );

        // Each vault should have unique ID for separate encryption
        expect(workVault.id, isNot(equals(personalVault.id)));

        // Vault metadata should be properly structured
        expect(workVault.name, 'Work Vault');
        expect(personalVault.name, 'Personal Vault');
      });

      test('Vault deletion requires confirmation', () {
        // Test that vault deletion has proper safeguards
        ...
        // Vault should exist and be deletable
        expect(vault.id, isNotEmpty);
        expect(vault.name, 'Test Vault');

        // In real implementation, deletion would require:
        // - Confirmation dialog
        // - Biometric authentication
        // - Secure data wiping
        expect(true, true); // Placeholder for deletion safeguards
      });
    });
    ...
    group('TOTP Integration', () {
      ...
      test('TOTP code generation produces valid codes', () {
        ...
        final code = generator.generateCode(config);
      ...
      test('TOTP time calculations are accurate', () {
        ...
        final generator = TOTPGenerator();
        final remainingSeconds = generator.getRemainingSeconds(config);
      ...
      test('Different TOTP algorithms are supported', () {
        ...
        final sha1Code = generator.generateCode(sha1Config);
        final sha256Code = generator.generateCode(sha256Config);
        final sha512Code = generator.generateCode(sha512Config);
    ...
    group('Import/Export Integration', () {
      ...
      test('Selective export works', () {
        // Test selective export by vault or category
        ...
        // Should be able to export specific categories
        expect(workAccounts.length, 1);
        expect(personalAccounts.length, 1);
        expect(workAccounts[0].url!.contains('company.com'), true);
        expect(personalAccounts[0].url!.contains('gmail.com'), true);
      });
    });
